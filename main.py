...
# ---------------------------------------------------
# 🔬 GPS RAW-UPLOAD DIAGNOSTIC
# ---------------------------------------------------
def diagnose_raw_image_metadata(file_bytes, filename, content_type=None):
    """Diagnostic only: inspect the exact bytes received by Flask.
    This does not modify the image or upload data.
    """
    print("━━━━━━━━━━ RAW IMAGE DIAGNOSTIC ━━━━━━━━━━", flush=True)
    try:
        print(f"📄 Filename: {filename}", flush=True)
        print(f"📦 Content-Type: {content_type}", flush=True)
        print(f"📏 Size: {len(file_bytes)} bytes", flush=True)
        print(f"🔢 First 16 bytes: {file_bytes[:16].hex()}", flush=True)

        if file_bytes[:3] == b"\xff\xd8\xff":
            print("🟢 JPEG signature detected", flush=True)
        elif file_bytes[:12].startswith(b"RIFF") and file_bytes[8:12] == b"WEBP":
            print("🟢 WEBP signature detected", flush=True)
        elif len(file_bytes) >= 12 and file_bytes[4:8] == b"ftyp":
            brand = file_bytes[8:12].decode("latin-1", errors="replace")
            print(f"🟢 ISO-BMFF/HEIF-style container detected (brand: {brand})", flush=True)
        else:
            print("⚠️ Unknown image/container signature", flush=True)

        has_exif_marker = b"Exif\x00\x00" in file_bytes
        print(
            "🟢 EXIF BLOCK EXISTS IN UPLOADED BYTES" if has_exif_marker
            else "🔴 NO EXIF BLOCK EXISTS IN UPLOADED BYTES",
            flush=True
        )

        # ExifRead: show every GPS-related key, not just the four keys
        try:
            tags = exifread.process_file(BytesIO(file_bytes), details=False)
            gps_keys = [str(k) for k in tags.keys() if "GPS" in str(k).upper()]
            print(f"🧾 ExifRead total tags: {len(tags)}", flush=True)
            if gps_keys:
                print("🛰️ ExifRead GPS-related tags:", flush=True)
                for key in gps_keys:
                    print(f"   • {key}: {tags.get(key)}", flush=True)
            else:
                print("🔴 ExifRead found ZERO GPS-related tags", flush=True)
        except Exception as e:
            print(f"⚠️ ExifRead diagnostic failed: {e}", flush=True)

        # Pillow: inspect decoded EXIF and GPS IFD safely
        try:
            img = Image.open(BytesIO(file_bytes))
            print(f"🖼️ Pillow format: {getattr(img, 'format', None)}", flush=True)
            exif = img.getexif()
            print(f"🧾 Pillow EXIF entries: {len(exif)}", flush=True)

            gps_ifd = {}
            try:
                gps_ifd = exif.get_ifd(34853) or {}
            except Exception as gps_err:
                print(f"⚠️ Pillow get_ifd(34853) failed: {gps_err}", flush=True)

            if gps_ifd:
                print("🛰️ Pillow GPS IFD entries:", flush=True)
                for key, value in gps_ifd.items():
                    name = GPSTAGS.get(key, str(key)) if isinstance(key, int) else str(key)
                    print(f"   • {name} ({key}): {value}", flush=True)
            else:
                print("🔴 Pillow GPS IFD is empty", flush=True)
        except Exception as e:
            print(f"⚠️ Pillow diagnostic failed: {e}", flush=True)

    except Exception as e:
        print(f"❌ RAW IMAGE DIAGNOSTIC ERROR: {e}", flush=True)
    finally:
        print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", flush=True)
...
def upload():
    ...
    try:
        ...
        for file in valid_files_to_process:
            ...
            diagnose_raw_image_metadata(file_bytes, orig_name, file.content_type)
