...
def _extract_with_pillow(file_bytes):
    ...
    Important: Pillow's ``exif.get(34853)`` can return the GPS IFD offset
    (an int) instead of the decoded GPS dictionary. ``get_ifd(34853)`` is
    the correct API for reading the GPS sub-IFD.
    """
    with Image.open(BytesIO(file_bytes)) as img:
        exif = img.getexif()

        # Decode the GPS sub-IFD. Do not use exif.get(34853), which may
        # return an integer offset and causes: 'int' object has no attribute 'get'.
        try:
